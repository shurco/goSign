...
func TestSubmitterHandler_ListAndNoAuthGuards(t *testing.T) {
	repo := newMemRepo[models.Submitter]()
	h := NewSubmitterHandler(repo, nil)
	...
	tests := []struct {
		...
		{
			name:       "Resend no auth returns 401",
			useAuth:    false,
			method:     http.MethodPost,
			path:       "/submitters/resend",
			body:       []byte(`{}`),
			wantStatus: http.StatusUnauthorized,
		},
		{
			name:       "Decline no auth returns 401",
			useAuth:    false,
			method:     http.MethodPost,
			path:       "/submitters/decline",
			body:       []byte(`{}`),
			wantStatus: http.StatusUnauthorized,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			h.RegisterRoutes(app.Group("/submitters"))
