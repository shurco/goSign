<template>
  <div class="relative cursor-crosshair select-none" :style="drawField ? 'touch-action: none' : ''">
    <img ref="image" loading="lazy" :src="`${image.url}/${image.filename}`" :width="width" :height="height" class="border rounded mb-4" @load="onImageLoad">
    <div class="top-0 bottom-0 left-0 right-0 absolute" @pointerdown="onStartDraw">
      <FieldArea v-for="(item, i) in areas" :key="i" :ref="setAreaRefs" :area="item.area" :field="item.field" :editable="editable"
        :default-field="defaultFields.find((f) => f.name === item.field.name)" @start-resize="resizeDirection = $event" @stop-resize="resizeDirection = null"
        @start-drag="isMove = true" @stop-drag="isMove = false" @remove="$emit('remove-area', item.area)" />
      <FieldArea v-if="newArea" :is-draw="true" :field="{ submitter_id: selectedSubmitter.id, type: drawField?.type || defaultFieldType }" :area="newArea" />
    </div>
    <div v-show="resizeDirection || isMove || isDrag || showMask || drawField" id="mask" ref="mask" class="top-0 bottom-0 left-0 right-0 absolute z-10"
      :class="{ 'cursor-grab': isDrag || isMove, 'cursor-nwse-resize': drawField, [resizeDirectionClasses[resizeDirection]]: !!resizeDirectionClasses }" @pointermove="onPointermove"
      @pointerdown="onStartDraw" @dragover.prevent @drop="onDrop" @pointerup="onPointerup" />
  </div>
</template>
