  ...
  <div :class="['signing-page', `theme-${theme}`]">
    ...
    <footer class="mt-8 border-t pt-4 text-center text-sm text-gray-500">
      <div v-if="branding.terms_url || branding.privacy_url" class="legal-links mb-2">
        <a v-if="branding.terms_url" :href="branding.terms_url" target="_blank" class="mx-2 hover:underline">
          Terms of Service
        </a>
        <a v-if="branding.privacy_url" :href="branding.privacy_url" target="_blank" class="mx-2 hover:underline">
          Privacy Policy
        </a>
      </div>

      <div v-if="branding.show_powered_by" class="powered-by">Powered by goSign</div>
    </footer>
  </div>
</template>
