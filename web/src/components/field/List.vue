<template>
  <div class="sticky top-0 z-10">
    <FieldSubmitter :model-value="selectedSubmitter.id" class="w-full rounded-lg bg-[#faf7f5]" :submitters="submitters" :editable="editable && !defaultSubmitters.length"
      @new-submitter="save" @remove="removeSubmitter" @name-change="save" @update:model-value="$emit('change-submitter', submitters.find((s: any) => s.id === $event))" />
  </div>


  <div ref="fields" class="mb-1 mt-2" @dragover.prevent="onFieldDragover" @drop="reorderFields">
    <Field v-for="field in submitterFields" :key="field.id" :data-uuid="field.id" :field="field" :type-index="fields.filter((f: any) => f.type === field.type).indexOf(field)"
      :editable="editable && (!dragField || dragField !== field)" :default-field="defaultFields.find((f: any) => f.name === field.name)" :draggable="editable"
      @dragstart="dragField = field" @dragend="dragField = null" @remove="removeField" @scroll-to="$emit('scroll-to-area', $event)" @set-draw="$emit('set-draw', $event)" />
  </div>

  <div v-if="editable && !onlyDefinedFields" class="grid grid-cols-3 gap-1 pb-2">
    <template v-for="(icon, type) in fieldIcons" :key="type">
      <button draggable="true" class="group flex items-center justify-center border border-dashed border-[#e7e2df] hover:border-[#291334]/20 w-full rounded relative"
        @dragstart="onDragstart({ type: type })" @dragend="$emit('drag-end')" @click="addField(type)">
        <div class="flex items-console group-hover:bg-[#efeae6]/50 transition-all cursor-grab h-full absolute left-0">
          <SvgIcon name="drag" width="18" height="18" class="my-auto" />
        </div>
        <div class="flex items-center flex-col px-2 py-2">
          <SvgIcon :name="icon" width="20" height="20" />
          <span class="text-xs mt-1">{{ fieldNames[type] }}</span>
        </div>
      </button>
    </template>
  </div>

  <div v-if="fields.length < 4 && editable" class="text-xs p-2 border border-[#efeae6] rounded">
    <ul class="list-disc list-outside ml-2 pl-2">
...
import { fieldNames as fieldNamesConst, fieldIcons as fieldIconsConst } from "@/components/field/constants.ts";
import { v4 } from "uuid";
...
const submitterDefaultFields = computed(() => props.defaultFields.filter((f: any) => {
  return !submitterFields.value.find((field: any) => field.name === f.name) && (!f.role || f.role === props.selectedSubmitter.name)
}))
...
function onFieldDragover(e: any) {
...
function removeSubmitter(submitter: any) {
...
function addField(type: any, area = null) {
  const field: any = {
    name: "",
    id: v4(),
