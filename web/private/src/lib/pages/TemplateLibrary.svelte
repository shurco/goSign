<script lang="ts">
  ...
  // Sync selectedFolderId from URL only when the path actually changes (avoids race on folder click)
  let lastSyncedPath = $state<string | null>(null);
  $effect(() => {
  ...
  const handleMoveTemplate = async (formData: Record<string, unknown>) => {
    ...
    try {
      ...
      if (response && (response.data || response.message)) {
        ...
        if (movedToFolderId) {
          selectedFolderId = movedToFolderId;
...
<div class="template-library">
  ...
  >
    {#snippet children(formData)}
      <div class="space-y-4">
        <div>
          <label for="createFolderName" class="mb-1 block text-sm font-medium text-gray-700"
            >{t("templates.folderName")}</label
          >
          <Input
            id="createFolderName"
  ...
  >
    {#snippet children(formData)}
      <div class="space-y-4">
        <div>
          <label for="renameFolderName" class="mb-1 block text-sm font-medium text-gray-700"
            >{t("templates.folderName")}</label
          >
          <Input
            id="renameFolderName"
  ...
  >
    {#snippet children(formData)}
      <div class="space-y-4">
        <div>
          <label for="createTemplateName" class="mb-1 block text-sm font-medium text-gray-700"
            >{t("templates.templateName")}</label
          >
          <Input
            id="createTemplateName"
            bind:value={
              () => (typeof formData.name === "string" ? formData.name : ""), (value) => (formData.name = value)
            }
        ...
        <div>
          <label for="createTemplateCategory" class="mb-1 block text-sm font-medium text-gray-700"
            >{t("templates.category")}</label
          >
          <Select id="createTemplateCategory" bind:value={formData.category} class="w-full">
            <option value="">{t("templates.allCategories")}</option>
            <option value="business">{t("templates.business")}</option>
            <option value="legal">{t("templates.legal")}</option>
            <option value="personal">{t("templates.personal")}</option>
        ...
        <div>
          <span class="mb-1 block text-sm font-medium text-gray-700"
            >{t("templates.uploadFromFile")} ({t("templates.optional")})</span
          >
          <label
  ...
  >
    {#snippet children(formData)}
      <div class="space-y-4">
        <div>
          <label for="editTemplateName" class="mb-1 block text-sm font-medium text-gray-700"
            >{t("templates.templateName")}</label
          >
          <Input
            id="editTemplateName"
        ...
        <div>
          <label for="editTemplateCategory" class="mb-1 block text-sm font-medium text-gray-700"
            >{t("templates.category")}</label
          >
          <Select id="editTemplateCategory" bind:value={formData.category} class="w-full">
            <option value="">{t("templates.allCategories")}</option>
            <option value="business">{t("templates.business")}</option>
            <option value="legal">{t("templates.legal")}</option>
            <option value="personal">{t("templates.personal")}</option>
  ...
  >
    {#snippet children(formData)}
      <div class="space-y-4">
        <div>
          <label for="moveTemplateFolder" class="mb-1 block text-sm font-medium text-gray-700"
            >{t("templates.selectFolder")}</label
          >
          <Select id="moveTemplateFolder" bind:value={formData.folder_id} class="w-full">
