<script lang="ts">
  ...
  interface Props {
    ...
    /** Prefill for typed tab (DocuSeal-style: initials auto-generated from submitter name). */
  ...
  onMount(() => {
    ...
    // DocuSeal-style auto-initials: prefill typed tab from submitter name when empty
    if (!value && defaultTypedText && showTyped && mode === "initials") {
      typedText = defaultTypedText;
      activeTab = "typed";
      emitTypedAsImage();
    }
