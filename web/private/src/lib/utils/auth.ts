...
/**
 * Register a client-side navigation function (SvelteKit `goto`) so auth
 * redirects don't trigger a full page reload.
 */
export function setAuthNavigate(navigate: (path: string) => void | Promise<void>): void {
  navigateFn = navigate;
}
...
/**
 * Notify listeners (admin guard) that the cached role must be re-fetched
 */
