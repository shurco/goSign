<script lang="ts">
  ...
  import SigningModeSelector from "@/components/field/SigningModeSelector.svelte";
  ...
  let submissions = $state<Signing[]>([]);
  let templates = $state<Template[]>([]);
  let folders = $state<{ id: string; name: string }[]>([]);
  let isLoading = $state(false);
  let showModal = $state(false);

  let expectedSubmittersCount = $state(0);
  let createSubmitters = $state<Array<{ name: string; email: string; phone: string }>>([]);
  let createSigningMode = $state<"sequential" | "parallel">("parallel");
  let createdLinks = $state<CreatedLink[]>([]);

  let linksModalOpen = $state(false);
  let activeSigning = $state<Signing | null>(null);
  let draggedSignerIndex = $state<number | null>(null);
  ...
  const createSubmittersForOrder = $derived(
    createSubmitters.map((s, idx) => ({
      id: String(idx),
  ...
  const modalTitle = $derived(t("submissions.create"));
  ...
  async function loadSubmissions(): Promise<void> {
    ...
    if (isLoading || loadSubmissionsPromise) {
    ...
    loadSubmissionsPromise = (async () => {
      try {
        const data = await apiGet("/signing-links");
        const payload = data.data || data;
        submissions = (payload.items || []) as Signing[];

        // Keep active signing in sync after refresh.
        if (activeSigning) {
          const updated = submissions.find((s) => s.submission_id === activeSigning?.submission_id) || null;
          activeSigning = updated;
        }
      } catch (error) {
    ...
    try {
      ...
      if (response && response.data) {
        ...
        if (Array.isArray(result)) {
          folders = result.map((f: any) => ({ id: f.id, name: f.name }));
        } else if (result.folders && Array.isArray(result.folders)) {
          folders = result.folders.map((f: any) => ({ id: f.id, name: f.name }));
        } else {
          folders = [];
        }
      }
  ...
  async function onTemplateChange(templateID: string): Promise<void> {
    ...
    try {
      ...
      createSubmitters = Array.from({ length: count }).map((_, idx) => ({
  ...
  function onCreateSubmitterOrder(ordered: Array<{ id: string; name: string }>): void {
    ...
    const current = [...createSubmitters];
    createSubmitters = indices.map((i) => current[i]).filter(Boolean);
  }

  function onSignerDragStart(event: DragEvent, index: number): void {
    draggedSignerIndex = index;
  ...
  function onSignerDragOver(event: DragEvent, cardIndex: number): void {
    ...
    if (draggedSignerIndex === null) {
      return;
    }
    const el = event.currentTarget as HTMLElement;
    const rect = el.getBoundingClientRect();
    const mid = rect.top + rect.height / 2;
    insertBeforeIndex = event.clientY < mid ? cardIndex : cardIndex + 1;
  }
  ...
  function onSignerDrop(event: DragEvent): void {
    ...
    const pos = insertBeforeIndex;
    if (pos === null) {
      draggedSignerIndex = null;
      insertBeforeIndex = null;
      return;
    }
    const arr = [...createSubmitters];
    const [item] = arr.splice(draggedSignerIndex, 1);
    const insertAt = draggedSignerIndex < pos ? pos - 1 : pos;
    arr.splice(insertAt, 0, item);
    createSubmitters = arr;
    draggedSignerIndex = null;
    insertBeforeIndex = null;
  }
  ...
  async function handleSubmit(formData: any): Promise<void> {
    try {
      ...
      if (expectedSubmittersCount <= 0 || createSubmitters.length !== expectedSubmittersCount) {
        alert(String(t("submissions.invalidTemplateSubmitters")));
        return;
      }

      const data = await apiPost("/signing-links", {
        template_id: formData.template_id,
        signing_mode: createSigningMode,
        submitters: createSubmitters.map((s, idx) => ({
          name: s.name || `${t("signing.signer")} ${idx + 1}`,
          email: s.email || "",
          phone: s.phone || ""
        }))
  ...
  async function copyAllCreatedLinks(): Promise<void> {
    ...
    await copyText(createdLinks.map((l) => l.full_url).join("\n"));
  }

  function openLinksModal(signing: Signing): void {
    activeSigning = signing;
    linksModalOpen = true;
  }
...
<div class="submissions-page">
  ...
  <div class="page-header">
    <div>
      ...
      <p class="page-subtitle">{t("submissions.description")}</p>
    </div>
  ...
  {#snippet cellTemplateName(item: unknown, value: string)}
    <button type="button" class="link text-left" onclick={() => openCompletedDocument((item as Signing).submission_id)}>
      {String(value || "")}
    </button>
  {/snippet}

  {#snippet cellStatus(item: unknown, value: string)}
    <button type="button" class="inline-flex cursor-pointer" onclick={() => openStatusHistory(item as Signing)}>
  ...
  {#snippet cellProgress(item: unknown)}
    <button
      type="button"
      class="flex cursor-pointer items-center gap-2 text-left"
      onclick={() => openStatusHistory(item as Signing)}
    >
      <progress
  ...
  {#snippet tableActions(item: unknown)}
    <div class="flex items-center justify-end gap-2">
      <button
        ...
        onclick={() => openLinksModal(item as Signing)}
      >
        <SvgIcon name="link" class={ICON_SVG_CLASS} />
      </button>

      {#if String((item as Signing).status) === "completed"}
        <button
          class={ICON_BUTTON_CLASS}
          type="button"
          title={t("common.download")}
          aria-label={t("common.download")}
          onclick={() => openCompletedDocument((item as Signing).submission_id)}
        >
  ...
  <ResourceTable
    ...
    {isLoading}
    searchable
    searchKeys={["template_name", "status"]}
    searchPlaceholder={t("submissions.searchSubmissions")}
    onPageChange={handlePageChange}
    cellSnippets={{
      template_name: cellTemplateName,
      status: cellStatus,
        ...
        <FormControl label={t("submissions.template")} error={errors.template_id}>
          ...
          >
            ...
            {#each templates as template (template.id)}
              <option value={template.id}>{getTemplateDisplayName(template)}</option>
            {/each}
          </Select>
        </FormControl>

        {#if expectedSubmittersCount > 0}
          <SigningModeSelector
            signingMode={createSigningMode}
            submitters={createSubmittersForOrder}
            editable={true}
            hideOrderList={true}
            onUpdateSigningMode={(mode) => {
              createSigningMode = mode as "sequential" | "parallel";
            }}
            onUpdateSubmitterOrder={(ordered) => onCreateSubmitterOrder(ordered as Array<{ id: string; name: string }>)}
          />

          <FormControl
            label={t("submissions.signers")}
            hint={createSigningMode === "sequential" && createSubmitters.length > 1
              ? t("submissions.signersOrderDragHint")
              : t("submissions.signersHint")}
          >
            <div class="space-y-4" role="presentation" ondragover={(e) => e.preventDefault()} ondrop={onSignerDrop}>
              {#each createSubmitters as s, idx (idx)}
                {#if insertBeforeIndex === idx && draggedSignerIndex !== null}
                  <div
                ...
                <div
                  ...
                  class="rounded-lg border border-[var(--color-base-300)] bg-[var(--color-base-100)]/50 p-4 transition-colors {createSigningMode ===
                    'sequential' && createSubmitters.length > 1
                    ? 'border-[var(--color-primary)]/50'
                    : ''} {draggedSignerIndex === idx ? 'ring-2 ring-[var(--color-primary)]/40' : ''}"
                  role="presentation"
                  ondragenter={(e) => e.preventDefault()}
                  ondragover={(e) => onSignerDragOver(e, idx)}
                  ondrop={(e) => {
                    e.preventDefault();
                    onSignerDrop(e);
                  }}
                >
                  <div
                    class="mb-3 flex items-center justify-between gap-2 {createSigningMode === 'sequential' &&
                    createSubmitters.length > 1
                      ? 'cursor-move'
                      : ''}"
                    role="presentation"
                    draggable={createSigningMode === "sequential" && createSubmitters.length > 1}
                    ondragstart={(e) => onSignerDragStart(e, idx)}
                    ondragend={onSignerDragEnd}
                  >
                    <span class="text-sm font-medium text-[var(--color-base-content)]">
                      {idx + 1}. {t("signing.signer")}
                      {idx + 1}
                    </span>
                    {#if createSigningMode === "sequential" && createSubmitters.length > 1}
                      <SvgIcon
                        name="drag"
                  ...
                  <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
                    {#each signerFieldKeys as field (field.key)}
                      <div>
                        <label
                          class="mb-1 block text-xs font-medium text-[var(--color-base-content)]/80"
                          for="signer-{idx}-{field.key}"
                        >
                          {t(field.labelKey)}
                        </label>
                        <input
                          id="signer-{idx}-{field.key}"
                          bind:value={s[field.key]}
                          type={field.type}
              ...
              {/each}
              {#if insertBeforeIndex === createSubmitters.length && draggedSignerIndex !== null}
                <div
                  class="h-1 flex-shrink-0 rounded-full bg-[var(--color-primary)] opacity-90"
                  aria-hidden="true"
                ></div>
  ...
  {#snippet linksActions(item: unknown)}
    <div class="flex items-center gap-2">
      <button
        ...
        title={t("signing.copyLink")}
        aria-label={t("signing.copyLink")}
        onclick={() => copyText(fullSignerUrl((item as any).slug))}
      >
    ...
    {#if activeSigning}
      <div class="space-y-4">
        <ResourceTable
          ...
          pageSize={100}
          cellSnippets={{
            index: linksCellIndex,
            signer: linksCellSigner,
            status: linksCellStatus,
            link: linksCellLink
          }}
...
<style>
  /* Page background comes from app-shell (--base-surf-page) */
  .submissions-page {
    min-height: 100%;
  }
</style>
