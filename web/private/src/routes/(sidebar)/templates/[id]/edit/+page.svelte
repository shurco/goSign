<script lang="ts">
  ...
  import { apiGet, apiPost, extractErrorMessage } from "@/services/api";
  ...
  function applyLoadedTemplate(tpl: any): void {
    ...
    for (const field of template.fields || []) {
      ...
      if (["radio", "multiple", "select"].includes(field.type) && !Array.isArray(field.options)) {
        field.options = [];
      }
    }
  ...
  async function putTemplateUpdate(payload: any, { keepalive }: { keepalive: boolean }): Promise<void> {
    ...
    // Manual fetch (not apiPut): keepalive lets the save finish after tab close/navigation.
...
<div class="edit-page flex h-full min-h-0 flex-col">
  <div class="page-header">
    <h1>
      {#if template}
        {t("templates.title")}
        <span class="edit-title-sep">→</span>
        <span>{template.name}</span>
      {:else}
        {t("templates.title")}
        <span class="edit-title-sep">→</span>
        <span>{t("templates.editor")}</span>
      {/if}
    </h1>
    ...
    {#if !loadingTemplate && template}
      <div class="page-actions">
        <button
          type="button"
          class="edit-save-btn"
          class:is-error={lastSaveError}
          class:is-saved={!isDirty && lastSavedAt && !lastSaveError}
        ...
        >
          ...
          {:else if !isDirty && lastSavedAt && !lastSaveError}
            <SvgIcon name="check-circle" class="h-4 w-4" />
          {:else if lastSaveError}
            <SvgIcon name="error-circle" class="h-4 w-4" />
          {/if}
          <span>
            {isSaving
              ? t("common.loading")
...
<style>
  ...
  /* Save button with status indicator (saved/error) over WS2-ghost */
  .edit-save-btn {
    display: inline-flex;
    align-items: center;
    gap: var(--space-6);
    height: var(--size-control-34);
