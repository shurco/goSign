<script lang="ts">
  ...
  async function loadAPIKeys(): Promise<void> {
    try {
      const data = await apiGet("/settings/api");
      apiKeys = Array.isArray(data.data) ? data.data : [];
    } catch (error) {
  ...
  async function saveAPIKey(formData: Record<string, unknown>): Promise<void> {
    try {
      const data = await apiPost("/settings/api", formData);
  ...
  async function deleteAPIKey(apiKey: APIKey): Promise<void> {
    ...
    try {
      await apiDelete(`/settings/api/${apiKey.id}`);
      await loadAPIKeys();
    } catch (error) {
