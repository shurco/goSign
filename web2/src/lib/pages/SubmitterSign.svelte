<script lang="ts">
  ...
  /** Drawer two-way value bridge (FieldFormDrawer bind:value). */
  let drawerValue = $state<string | boolean | string[] | undefined>(undefined);
  ...
  // Sync drawer value from formData when active field changes
  $effect(() => {
    const id = expandedFieldId;
    if (id === null) {
      drawerValue = undefined;
      return;
    }
    drawerValue = formData[id];
  });

  // Sync drawer value back to formData
  $effect(() => {
    const id = expandedFieldId;
    if (id === null) {
      return;
