...
describe("useConditions - Condition Evaluation Logic", () => {
  ...
  describe("Equals Operator", () => {
    it("should return true when field value equals condition value", () => {
      ...
      const { evaluateCondition } = useConditions(
        () => fields,
        () => formData
      );
  ...
  describe("AND Logic in Condition Groups", () => {
    it("should return true when all conditions in AND group are met", () => {
      ...
      const { evaluateGroup } = useConditions(
        () => fields,
        () => formData
      );
  ...
  describe("Field State Calculation - Show Action", () => {
    it("should show field when condition is met with show action", () => {
      ...
      const { getFieldState } = useConditions(
        () => fields,
        () => formData
      );
      const state = getFieldState(fields[2]);
  ...
  describe("Field States Computed", () => {
    it("should compute states for all fields", () => {
      ...
      expect(conditions.fieldStates).toHaveProperty("field_1");
      expect(conditions.fieldStates).toHaveProperty("field_2");
      expect(conditions.fieldStates).toHaveProperty("field_3");
      expect(conditions.fieldStates.field_3.visible).toBe(true);
    });
    ...
    it("should update states reactively when formData changes", () => {
      ...
      formData.field_1 = "different";
      expect(conditions.fieldStates.field_3.visible).toBe(false);
    });
  });

  describe("Edge Cases", () => {
    it("should handle field with no condition groups", () => {
      fields[0].condition_groups = undefined;

      const { getFieldState } = useConditions(
        () => fields,
        () => formData
      );
      const state = getFieldState(fields[0]);
      expect(state.visible).toBe(true);
      expect(state.required).toBe(false);
