<script lang="ts">
  ...
  const submitterColors = $derived.by(() => {
    ...
    submitters.forEach((submitter, index) => {
      ...
      colors[submitter.id as string] = subColors[(submitter.colorIndex as number) % subColors.length];
    });
    return colors;
  });
    ...
    <div class="flex items-end space-x-2">
      ...
      >
        <div class="flex items-center space-x-2">
          {#if selectedSubmitter}
            <span class="h-3 w-3 flex-shrink-0 rounded-full {getSubmitterColor(selectedSubmitter)}" />
              ...
              <li>
                ...
                >
                  <span class="flex items-center py-1">
                    <span class="mr-3 ml-1 h-3 w-3 rounded-full {getSubmitterColor(submitter)}" />
                    <span>
                      {submitter.name}
                    </span>
                  </span>
  ...
  <div bind:this={dropdownRef} class="dropdown {className}" {onclick} use:clickOutside={() => dropdown.close()}>
    ...
    {:else}
      ...
      >
        <div class="flex items-center space-x-2">
          {#if selectedSubmitter}
            <span class="h-3 w-3 rounded-full {getSubmitterColor(selectedSubmitter)}" />
            <!-- svelte-ignore a11y_click_events_have_key_events -->
            <!-- svelte-ignore a11y_no_static_element_interactions -->
            <div onclick={(e) => e.stopPropagation()}>
              <Contenteditable
