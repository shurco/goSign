...
var RSATestCert = `-----BEGIN CERTIFICATE-----
...
func TestGetCertificate(t *testing.T) {
	...
	t.Run("Test created certificate match with given information", func(t *testing.T) {
		...
		if c.Subject.CommonName != "cert" {
			t.Fatalf("got %v, want cert", c.Subject.CommonName)
		}

		if c.Subject.Organization[0] != "cert" {
			t.Fatalf("got %v, want cert", c.Subject.Organization[0])
		}
